...
class BaseCAM:
    ...
    def get_cam_image(self,
        ...
        weighted_activations = weights[:, :, None, None] * activations  #权重乘特征图
        if eigen_smooth:
            cam = get_2d_projection(weighted_activations)
        else:
            cam = weighted_activations.sum(axis=1)   #特征加权
        return cam
    ...
    def compute_cam_per_layer(self, input_tensor: torch.Tensor, targets: List[torch.nn.Module], eigen_smooth: bool) -> np.ndarray:
        ...
        #target_size = self.get_target_width_height(input_tensor)   #个人修改注释
        ...
        for i in range(len(self.target_layers)):
            ...
            #scaled = scale_cam_image(cam, target_size)   #个人修改注释
