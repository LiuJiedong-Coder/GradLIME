...
def computer_score(model, input, black_pred_id, attr_arry, metric_arry, device):
    ...
    for metric in metric_arry:
        ...
        for attr in attr_arry:
            score = MyMetrics.compute_Complexity(model, input, black_pred_id, attr, metric, device)
...
def write_to_file(cor_black, sample_num, sample_score, class_name):
    with open('results/Complexity-GradLIME.txt', 'a') as file:
...
if __name__ == '__main__':
    ...
    for class_name in classes:
        ...
        for img_path in range(len(loaded_images)):
            ...
            temp, mask = explanation_org.get_image_and_mask(explanation_org.top_labels[0], positive_only=True, num_features=num_features, hide_rest=False)
            ...
            metrics = ['Sparseness', 'Complexity', 'EffectiveComplexity']
